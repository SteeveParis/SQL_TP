...
/
    --Question 3 :
CREATE OR REPLACE TRIGGER TRGPREVENT_MODIFICATION_ROBOTS_FROM_FACTORY
...
BEGIN
    ...
    -- Compter le nombre de tables de travailleurs respectant le format WORKERS_FACTORY<N>
    SELECT COUNT(*) INTO v_num_workers_tables
    FROM user_tables
    WHERE table_name LIKE 'WORKERS_FACTORY_%' ESCAPE '';

    -- Vérifier si le nombre d'usines est égal au nombre de tables de travailleurs
    IF v_num_factories <> v_num_workers_tables THEN
        -- Lever une erreur si la condition n'est pas respectée
